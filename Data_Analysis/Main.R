...
?Maaslin2
setwd("/Users/parsa/Downloads/Step 3 Analysis/")
metadata=read.csv("metadata.tsv",sep="\t", header = TRUE, row.names = 1)
Path_Abundance=read.csv("Merged_Pathabundance_Table_CPM_meta.tsv",row.names = 1,sep='\t',header = TRUE)
Rxn_Abundance=read.csv("rxn-cpm.txt",row.names = 1,sep='\t',header = TRUE)
